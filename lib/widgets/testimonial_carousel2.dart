...
class TestimonialCarousel2 extends StatefulWidget {
  @override
  _TestimonialCarousel2State createState() => _TestimonialCarousel2State();
}

class _TestimonialCarousel2State extends State<TestimonialCarousel2> {
  ...
  final List<Testimonial> testimonials = [
    ...
    Testimonial(
      ...
      testimonial: "This platform has exceeded my expectations in every way.",
      rating: 5,
    ),
  ];
  ...
  void _startAutoScroll() {
    _timer = Timer.periodic(const Duration(milliseconds: 1500), (timer) {
      if (_pageController.hasClients) {
        int prevPage = currentPage - 1; // Move left (reverse)

        if (prevPage < 0) {
          // If at the first card, smoothly move to last
          prevPage = testimonials.length - 1;
        }
